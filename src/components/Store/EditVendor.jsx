...
const EditVendor = ({ vendor, onClose, onSubmit }) => {
  ...
  const [originalGstStatus, setOriginalGstStatus] = useState("")
  ...
  useEffect(() => {
    ...
    setGstStatus(vendor.gstStatus || "")
    setOriginalGstStatus(vendor.gstStatus || "")
    setGstNumber(vendor.gstNumber || "")
    setPanNumber(vendor.panNumber || "")
    setMsmeStatus(vendor.msmeStatus || "NOT_REGISTERED")
    setAddress(vendor.address || "")
    setVendorType(vendor.vendorType || "")
    setStatus(vendor.status || "ACTIVE")
    setSelectedMainGroupIds((vendor.productMainGroups || []).map((g) => g.id))
  ...
  const gstStatusChangedFromRegistered = originalGstStatus === "REGISTERED" && gstStatus === "UNREGISTERED"
  ...
  const addContact = () =>
    setContacts((prev) => [...prev, { id: null, name: "", mobileNumber: "", email: "", designation: "", isPrimary: false }])

  const removeContact = (index) => {
    if (contacts[index].isPrimary) return
  ...
  const validate = () => {
    ...
    } else if (!contacts.some((c) => c.isPrimary)) {
      errors.contacts = "One contact must be marked primary"
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ASSUMPTION: storeService.updateVendor(id, data) is a plain
      // axios.put passthrough, matching the confirmed createVendor pattern.
      // If it's not, this is the one line that needs to change.
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 p-4" onClick={onClose}>
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {gstStatusChangedFromRegistered && (
            <div className="flex items-start gap-2 bg-yellow-50 border border-yellow-200 text-yellow-800 text-sm rounded-lg p-3">
              <FiAlertTriangle className="flex-shrink-0 mt-0.5" size={16} />
              <span>
                Changing to Unregistered will remove the saved GST number ({vendor?.gstNumber}) on save.
                Make sure the PAN above is correct — it will become the vendor's only tax ID.
              </span>
            </div>
          )}
          ...
          <div>
            ...
            {fieldErrors.contacts && <p className="text-red-600 text-xs mb-2">{fieldErrors.contacts}</p>}

            {contacts.map((c, i) => (
              <div key={c.id ?? `new_${i}`} className="border border-gray-200 rounded-lg p-4 mb-3">
                <div className="flex justify-between items-center mb-2">
                  {c.isPrimary ? (
                  ...
                  {!c.isPrimary && (
                    <button type="button" onClick={() => removeContact(i)} className="text-red-500 hover:text-red-700">
                      <FiTrash2 size={16} />
                    </button>
                  )}
                </div>
