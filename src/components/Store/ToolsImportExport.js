...
const ToolsImportExport = ({ onClose, onSuccess }) => {
  ...
  const handleImport = async () => {
    ...
    try {
      const result = await storeService.importTools(importFile)
      if (result.hasErrors) {
        setImportMessage(
        ...
        setImportMessage("Tools imported successfully!")
      }
  ...
  const handleExport = async () => {
    ...
    try {
      const data = await storeService.exportTools()
  ...
  return (
    ...
    >
      ...
      <h2 className="text-2xl font-bold text-gray-800 mb-6">Import/Export Tools</h2>

      {/* Import Section */}
      <div className="mb-8 p-4 border border-gray-200 rounded-lg">
        <h3 className="text-xl font-semibold text-gray-700 mb-4 flex items-center gap-2">
          <FiUpload /> Import Tools
        </h3>
        <input
          type="file"
          accept=".xlsx, .xls"
...
export default ToolsImportExport