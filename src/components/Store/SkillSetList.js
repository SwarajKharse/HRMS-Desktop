...
const SkillSetList = () => {
  ...
  const [showAddSkillSetDialog, setShowAddSkillSetDialog] = useState(false)
  const [selectedSkillSet, setSelectedSkillSet] = useState(null)
  const [showForm, setShowForm] = useState(false)
  const [successMessage, setSuccessMessage] = useState(null)

  // State for filters (only search query for skill sets)
  ...
  const handleDetails = (e, id) => {
    e.stopPropagation()
    const skillSet = skillSets.find((item) => item.id === id)
    setSelectedSkillSet(skillSet)
    setShowForm(true)
  }

  // Fetch skill sets with filters and pagination
  const fetchSkillSets = async (page = 0, size = 10, search = "") => {
    try {
      setLoading(true)
      setError(null)
      const response = await storeService.getSkillSets(page, size, search)
  ...
  // Initial data load
  useEffect(() => {
    fetchSkillSets(currentPage, itemsPerPage, searchQuery)
  }, [])

  // Handle search with debounce
  useEffect(() => {
    const timer = setTimeout(() => {
      fetchSkillSets(0, itemsPerPage, searchQuery)
    }, 500)
  ...
  const handlePageChange = (pageNumber) => {
    fetchSkillSets(pageNumber, itemsPerPage, searchQuery)
  }

  // Reset all filters (only search query for skill sets)
  const resetFilters = () => {
    setSearchQuery("")
    fetchSkillSets(0, itemsPerPage, "")
  }

  const handleEditSkillSet = async () => {
    try {
  ...
  return (
    <div className="flex flex-col gap-6 p-4 md:p-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white rounded-xl p-4 md:p-6 shadow-sm">
        <h1 className="text-xl md:text-2xl font-bold text-gray-800">Skill Sets</h1>
        <div className="flex items-center gap-4">
          <button
            onClick={() => setShowAddSkillSetDialog(true)}
      ...
      <div className="grid grid-cols-1 gap-6">
        {/* Filters - Desktop (simplified for SkillSet) - Removed */}
        ...
        {/* Filters - Mobile (simplified for SkillSet) - Removed */}
        {/* {showMobileFilters && (
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 mb-6">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search skill sets by name..."
                value={searchQuery}
          ...
          <div className="bg-white rounded-xl shadow-sm overflow-hidden">
            {loading && skillSets.length === 0 ? (
              <div className="flex justify-center items-center p-8">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600"></div>
              </div>
            ) : skillSets.length === 0 ? (
              <div className="text-center p-8 text-gray-500">No skill sets found matching your criteria</div>
            ) : (
                ...
                <table className="min-w-full divide-y divide-gray-200 table-fixed">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {skillSets.map((skillSet) => (
                      <tr key={skillSet.id} className="hover:bg-gray-50">
                        <td className="px-6 py-4">
                          <div
                            className="text-sm font-medium text-gray-900 truncate max-w-[300px]"
                            title={skillSet.skillset_name}
                          >
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center">
                            <button
                              ...
                              onClick={(e) => handleDetails(e, skillSet.id)}
                              title="Details"
                            >
            ...
            {totalPages > 1 && (
              <div className="px-4 py-3 flex flex-col sm:flex-row justify-between items-center border-t border-gray-200 bg-gray-50">
                <div className="text-sm text-gray-700 mb-2 sm:mb-0">
                  Showing <span className="font-medium">{skillSets.length}</span> of{" "}
                  <span className="font-medium">{totalItems}</span> skill sets
                </div>
      ...
      {showImportExportDialog && (
        ...
        >
          <SkillSetImportExport
            onClose={() => setShowImportExportDialog(false)}
            onSuccess={() => {
              setShowImportExportDialog(false)
              fetchSkillSets(currentPage, itemsPerPage, searchQuery)
              setSuccessMessage("Skill Sets imported successfully!")
      ...
      {/* Add Skill Set Dialog */}
      {showAddSkillSetDialog && (
        <div
          className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 p-4"
          onClick={() => {
            setShowAddSkillSetDialog(false)
          }}
        >
          <AddSkillSet
            onClose={() => setShowAddSkillSetDialog(false)}
            onSuccess={() => {
              setShowAddSkillSetDialog(false)
              fetchSkillSets(currentPage, itemsPerPage, searchQuery)
              setSuccessMessage("Skill Set added successfully!")
              setTimeout(() => setSuccessMessage(null), 5000)
            }}
          />
        </div>
