...
function TerminationForm({ employee, onClose, onSubmit }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await terminationService.applyTermination(employee.id, formData.reason)
      onSubmit && onSubmit()
      onClose()
    } catch (err) {
      ...
      >
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center space-x-2">
            <FiUserX className="text-red-500 w-6 h-6" />
            <h2 className="text-xl font-semibold">Issue Termination</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end space-x-3 mt-6">
            ...
            >
              {loading ? "Applying Termination..." : "Apply Termination"}
            </button>
          </div>
        </form>
      </motion.div>
