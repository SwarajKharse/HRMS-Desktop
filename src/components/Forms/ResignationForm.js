...
function ResignationForm({ employee, onClose, onSubmit }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await resignationService.applyResignation(employee.id, formData.reason)
      onSubmit && onSubmit()
      onClose()
    } catch (err) {
      ...
      >
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center space-x-2">
            <FiLogOut className="text-gray-500 w-6 h-6" />
            <h2 className="text-xl font-semibold">Apply Resignation</h2>
          </div>
