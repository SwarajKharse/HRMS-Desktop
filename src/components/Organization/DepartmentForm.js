...
function DepartmentForm({ department, departments, orgId, onClose, onSubmit }) {
  ...
  useEffect(() => {
    if (department) {
      setFormData({
        ...department,
        parentDepartment: department.parentDepartment?.id || null,
      })
    }
  }, [department]);
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: name === "parentDepartment" ? (value ? { id: value } : null) : value,
    }))
    if (error) setError("")
  };
    ...
    >
      ...
      >
        <div className="flex justify-between items-center p-6 border-b">
          <h2 className="text-xl font-semibold">{department ? "Edit Department" : "Add New Department"}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <select
              ...
              value={formData.parentDepartment?.id || ""}
              onChange={handleChange}
              className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2"
            >
