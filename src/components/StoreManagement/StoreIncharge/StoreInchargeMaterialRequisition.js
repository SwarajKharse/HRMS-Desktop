...
export default function StoreInchargeMaterialRequisition() {
  const { user } = useAuth()
  let userId = 1
  if (user) {
  ...
  const fetchMaterialRequisitions = useCallback(async () => {
    ...
    try {
      const filterParams = {
        ...
        mtrDateFrom: filters.mtrDateFrom || null,
        mtrDateTo: filters.mtrDateTo || null,
      }

      const data = await materialRequisitionService.getMaterialRequisitionsByStoreIncharge(
        currentUserId,
        filterParams,
        currentPage,
        pageSize,
