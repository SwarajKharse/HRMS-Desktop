      ...
      try {
        ...
        if (dispatchHistoryData && Array.isArray(dispatchHistoryData)) {
          ...
          Object.keys(dispatchHistoryMap).forEach((key) => {
            dispatchHistoryMap[key].sort((a, b) => new Date(b.recordedAt).getTime() - new Date(a.recordedAt).getTime())
          })
        }
  ...
  const handleSave = async () => {
    try {
      ...
      boqItems.forEach((item) => {
        ...
        // Only save INSTALLATION and DISPATCH plans from this component
        ...
        if (dispatchPlan[itemKey]?.date) {
          plansToSave.push({
            ...
            boqCategoryItemId: boqCategoryItemId,
            planType: "DISPATCH",
            planDate: dispatchPlan[itemKey].date,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="max-h-[90vh] w-full max-w-6xl overflow-auto rounded-lg bg-white p-4 shadow-lg">
        ...
        <div className="mb-6 flex items-center justify-between border-b pb-4">
          <h2 className="text-xl font-bold text-gray-800">Project Dispatch Plan</h2>
