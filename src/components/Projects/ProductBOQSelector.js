...
function ProductBOQSelector({ projectId, onSave, existingBOQ = null, isEditMode = false }) {
  ...
  useEffect(() => {
    if (isEditMode && existingBOQ && existingBOQ.items) {
  ...
  const handleQuantityChange = (id, qty) => {
    const updatedProducts = selectedProducts.map(product => {
      if (product.id === id) {
        return { ...product, qty: parseInt(qty) || 0 };
      }
      return product;
    });
  ...
  const handleSaveBOQ = async () => {
    ...
    const invalidProducts = selectedProducts.filter(p => !p.qty || p.qty <= 0);
                      ...
                      <div className="flex justify-between items-start">
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-gray-500 mt-1">
                            HSN: {product.hsn_code || 'N/A'} | UOM: {product.uom || 'N/A'} | Qty Available: {product.product_qty || '0'}
                          </div>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead>
                  <tr>
                    ...
                    <th className="px-3 py-2 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Category Hierarchy</th>
                    <th className="px-3 py-2 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Make</th>
                    <th className="px-3 py-2 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">UOM</th>
                      ...
                      <tr key={product.id} className={index % 2 === 0 ? "bg-gray-50" : ""}>
                        ...
                        <td className="px-3 py-2">
                          <input
                            type="number"
                            min="1"
                            max={parseInt(product.product_qty) || 999999}
                            value={product.qty || 1}
