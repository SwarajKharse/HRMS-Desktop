...
function ProductBOQSelector({ projectId, onSave, leadProductTypes, existingBOQ = null, isEditMode = false }) {
  ...
  const handleSaveBOQ = async () => {
    ...
    const invalidProducts = allProducts.filter(
      (p) =>
        ...
        !p.supply_rate ||
        Number.parseFloat(p.supply_rate) < 0 ||
        !p.installation_rate ||
  ...
  const totalSupplyAmount = allProductsFlat.reduce((sum, product) => sum + (product.supply_amount || 0), 0)
  const totalInstallationAmount = allProductsFlat.reduce((sum, product) => sum + (product.installation_amount || 0), 0)
                                ...
                                <tr key={product.id} className={index % 2 === 0 ? "bg-gray-50" : ""}>
                                  ...
                                  <td className="px-3 py-2">
                                    <input
                                      ...
                                      onChange={(e) => {
                                        const value = e.target.value
                                        if (value === "" || /^\d*\.?\d*$/.test(value)) {
                                          handleProductFieldChange(category.id, product.id, "supply_rate", value)
                                        }
                                      }}
                                      onBlur={(e) => {
                                        if (e.target.value === "") {
                                          handleProductFieldChange(category.id, product.id, "supply_rate", "0")
                                        }
                                      }}
                                  ...
                                  <td className="px-3 py-2">
                                    <input
                                      ...
                                      onChange={(e) => {
                                        const value = e.target.value
                                        if (value === "" || /^\d*\.?\d*$/.test(value)) {
                                          handleProductFieldChange(category.id, product.id, "installation_rate", value)
                                        }
                                      }}
                                      onBlur={(e) => {
                                        if (e.target.value === "") {
                                          handleProductFieldChange(category.id, product.id, "installation_rate", "0")
                                        }
                                      }}
                                  ...
                                  <td className="px-3 py-2">
                                    <input
                                      type="text"
                                      readOnly
                                      value={product.supply_amount.toFixed(2)}
                                  ...
                                  <td className="px-3 py-2">
                                    <input
                                      type="text"
                                      readOnly
                                      value={product.installation_amount.toFixed(2)}
