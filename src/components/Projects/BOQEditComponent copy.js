...
function BOQEditComponent({ projectId, projectName, existingBOQ, onSave, onClose }) {
  ...
  const SkillsetDropdown = ({ productId, onSelect }) => {
    ...
    const filteredSkillsets = availableSkillsets.filter(
      (skillset) =>
        (skillset.name && skillset.name.toLowerCase().includes(searchTerm.toLowerCase())) ||
        (skillset.skillset_name && skillset.skillset_name.toLowerCase().includes(searchTerm.toLowerCase())),
    )
  ...
  const ToolsDropdown = ({ productId, onSelect }) => {
    ...
    const filteredTools = availableTools.filter(
      (tool) =>
        ...
        (tool.tool_name && tool.tool_name.toLowerCase().includes(searchTerm.toLowerCase())),
    )
  ...
  const saveChanges = async () => {
    try {
      ...
      // Use a single API endpoint for both creating and updating
