...
function SSENewLeads() {
  ...
  const fetchLeads = useCallback(async () => {
    try {
      ...
      const data = await leadService.getLeadsAssignesToSSE(
        userId,
        page,
        leadsPerPage,
        searchQuery,
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    {["Lead ID", "Lead Priority", "Date Received", "Lead Type", "Product Type", "Actions"]
                      .filter(Boolean)
                      .map((header) => (
                        <th
                          key={header}
                  ...
                  {unassignedleads.length === 0 ? (
                    ...
                    unassignedleads.map((lead) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <div className="text-xs font-medium text-gray-900">{formatDate(lead.lead_recieved)}</div>
                        </td>
                        <td className="px-6 py-4">
                  ...
                  {unassignedleads.map((lead) => (
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-y-2 text-xs">
                        ...
                        <div className="font-medium">{formatDate(lead.lead_recieved)}</div>
