...
export const projectService = {
  ...
  updateStockAlloted: async (projectId, requisitionId, lineId, itemKind, stockAlloted, remarks, currentUserId) => {
    try {
      const response = await axios.put(
        `${API_URL}/projects/${projectId}/requisitions/${requisitionId}/lines/${lineId}/stock`,
        { itemKind, stockAlloted, remarks, currentUserId },
        getAuthHeaders()
      )
      return response.data
    } catch (error) {
...
uploadNocFile: async (projectId, file) => {
  ...
  /* generatePOPDF: async (leadId, boqWithGST, documentType = "po_document") => {
    try {
      const payload = {
        ...boqWithGST,
        document_type: documentType,
      }
      const response = await axios.post(`${API_URL}/projects/${leadId}`, payload, getAuthHeaders())
      return response.data
    } catch (error) {
      console.error("Error generating PO PDF:", error)
      throw error
