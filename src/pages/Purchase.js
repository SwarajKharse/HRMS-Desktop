...
function Projects() {
  ...
  const isManager = () => designation.includes("purchase-manager")
  const isSubordinate = () => designation.includes("purchaser")

  const shouldShowMainTabs = () => isManagement() || isManager()

  const getAvailableMainTabs = () => {
    if (isManagement() || isManager()) return ["PurchaseManager", "Purchaser"]
    return []
  }
  ...
  useEffect(() => {
    sessionStorage.setItem("purchaseActiveTab", activeTab)
  }, [activeTab])
